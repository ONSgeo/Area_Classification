...
def create_radial_plots_wrapper(
    ...
    create_radial_plots(
        config, uk_std_cluster_means, level="UK", domain_colours=config["domain_colours"]
    )

    # Create radial plots for groups against their parent (groups)
    create_radial_plots(
        config, combined_group_means, level="group", domain_colours=config["domain_colours"]
    )

    # Create radial plots for subgroups against their parent (groups)
    create_radial_plots(
        config, combined_subgroup_means, level="subgroup", domain_colours=config["domain_colours"]
    )
...
def create_radial_plots(config, dataframe, level, domain_colours):
    ...
    for _, row in dataframe.iterrows():
        ...
        bottom_label_radii = [
            ...
            4.2,
            4.2,
        ]  # Closest to bottom (angle=pi), then next, etc.
        ...
        bottom_indices_sorted = sorted(bottom_candidates, key=lambda x: abs(x[1] - (3 * np.pi / 2)))[
            :6
        ]

        # Build maps for radii/tick_end
        top_map = {
            idx: (r, t) for (idx, _), r, t in zip(top_indices_sorted, top_label_radii, top_tick_ends)
        }
        bottom_map = {
            idx: (r, t)
            for (idx, _), r, t in zip(bottom_indices_sorted, bottom_label_radii, bottom_tick_ends)
        }
        ...
        for i, col in enumerate(feature_cols):
            ...
            ax.text(
                angle, label_radius, label, fontsize=18, color="black", ha=ha, va="center", zorder=11
            )
        ...
        if level == "group":
            ...
            ax.set_title(
                f"{row['cluster']} {row['hierarchy_level']} (UK mean)",
                size=26,
                pad=80,
                weight="bold",
            )
