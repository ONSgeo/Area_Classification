...
def cluster_variable_means(config, restructured_cluster_table, standardised_data):
    ...
    uk_std_cluster_means = (
        long_data.groupby(["variable_name", "hierarchy_level", "cluster"])
        .mean("variable_value")
        .reset_index()
    )

    # Pivot the data to create the wide format
    uk_std_cluster_means = uk_std_cluster_means.pivot(
        index=["cluster", "hierarchy_level"], columns="variable_name", values="variable_value"
    ).reset_index()
