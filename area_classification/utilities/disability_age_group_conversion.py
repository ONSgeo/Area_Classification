...
def convert_disability_age_group_scotland(filepath: str, config: dict) -> pd.DataFrame:
    ...
    for index, row in df.iterrows():
        if str(row.iloc[0]).strip().lower() == "sex":
            ...
            if index == 0:
                ...
                council_area = (
                    df.iloc[index - 2, 0] if index - 2 >= 0 else None
                )  # Get value from two rows above
            ...
            first_element_list = [
                int(s.split()[0]) if isinstance(s, str) and len(s.split()) > 0 else ""
                for s in age_band_list
            ]
            ...
            columns_to_convert = council_df.columns[
                2:
            ]  # Select all columns starting from the 3rd column onward
            ...
            age_band_names_and_bools = define_age_bands_and_bools(
                council_df, lower_age_band_col="lower_age_band"
            )
    ...
    result_df["CA19"] = (
        result_df["CA19"].str.strip().str.lower().map(lookup_dict).fillna(result_df["CA19"])
    )
...
def convert_disability_age_group_northern_ireland(filepath: str, config: dict) -> pd.DataFrame:
    ...
    ni_long_df = ni_df.melt(
        id_vars=["geography code", "geography"], var_name="age_disability_group", value_name="count"
    )

    # Extract lower age band from the group name
    ni_long_df["lower_age_band"] = (
        ni_long_df["age_disability_group"].str.extract(r"(\d*)").replace("", None).astype(float)
    )
    ...
    disability_condition = ni_long_df["age_disability_group"].str.contains(
        r"limited a l.*", case=False, regex=True
    )
    non_disability_condition = ni_long_df["age_disability_group"].str.contains(
        r"not limited", case=False, regex=True
    )
    ...
    for (geo_code, geo_name), group_df in ni_long_df.groupby(["geography code", "geography"]):
        for age_band_name, condition in age_band_names_and_bools.items():
            new_row = {
                ...
                "total_population": group_df.loc[
                    condition & (disability_condition | non_disability_condition), "count"
                ].sum(),
            }
            result_df_list.append(new_row)
