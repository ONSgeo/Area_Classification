...
def quality_checks_all_dfs(df):
    ...
    auto_check_type = (
        input("Do you want to run a basic or a full auto check? (basic/full): ").strip().lower()
    )
    ...
    if auto_check_type == "basic":
        ...
        while True:
            ...
            run_full = (
                input("Do you want to complete the full auto check? (yes/no) ").strip().lower()
            )
            if run_full == "yes":
    ...
    if auto_check_type == "full":
        ...
        run_user_input = (
            input("Do you want to complete the user input checks? (yes/no) ").strip().lower()
        )
...
def user_input_qa(df):
    ...
    check_shape = (
        input("Do you want to check the number of rows and columns? (yes/no): ").strip().lower()
    )
    if check_shape == "yes":
    ...
    check_ranges = (
        input("Do you want to check for values outside a specific range? (yes/no): ").strip().lower()
    )
    if check_ranges == "yes":
        # Ask the user if they want to check ranges for the entire dataframe or only specific columns
        range_scope = (
            input("Do you want to check the ranges for the entire dataframe? (yes/no): ")
            .strip()
            .lower()
        )
        if range_scope == "yes":
            while True:
                ...
                if outside_range.any().any():
                    ...
                    out_of_range_counts = (
                        (df_numeric < min_expected) | (df_numeric > max_expected)
                    ).sum()
                ...
                specific_check = (
                    input("Do you want to check the range of any specific columns? (yes/no): ")
                    .strip()
                    .lower()
                )
        ...
        if range_scope == "no":
            while True:
                ...
                columns_to_check = input(
                    "Enter the column heading(s) you want to check, separated by commas: "
                ).split(",")
                ...
                for col in columns_to_check:
                    if col in df.columns:
                        min_expected = float(
                            input(f"Enter the minimum expected value for column '{col}': ")
                        )
                        max_expected = float(
                            input(f"Enter the maximum expected value for column '{col}': ")
                        )

                        # Convert column to numeric for comparison
                        col_numeric = pd.to_numeric(df[col], errors="coerce")
                        out_of_range_mask = (col_numeric < min_expected) | (
                            col_numeric > max_expected
                        )
                ...
                more_ranges = (
                    input("Do you want to check the range of any more columns? (yes/no): ")
                    .strip()
                    .lower()
                )
                if more_ranges == "yes":
    ...
    if check_duplicates == "yes":
        ...
        duplicate_scope = (
            input("Do you want to check every column for duplicate values? (yes/no): ")
            .strip()
            .lower()
        )
        if duplicate_scope == "yes":
            ...
            while True:
                ...
                more_duplicates = (
                    input(
                        "Do you want to check for duplicate values in any more columns? (yes/no): "
                    )
                    .strip()
                    .lower()
                )
