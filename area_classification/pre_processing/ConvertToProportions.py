    ...
    for file in csv_files:
        ...
        for var in table_vars:
            ...
            if var not in ignore_vars:
                ...
                table[var] = table[var] / table[total_var]
        
        out_name = table_name + '_proportions.csv'
        table.to_csv(os.path.join(csv_folder_path, out_name), index=None)
        
        print(f"{table_name} converted to proportion of total")
...
def validate_output(csv_folder_path: Union[str, Path],
    ...
    required variables are within range [0,1].
    ...
    ------
    ValueError
        If variable values are outside [0,1] range
    
    """
    
    processed_csv_files = [file for file in os.listdir(csv_folder_path) if '_proportions.csv' in file]
    
    for file in processed_csv_files:
        
        table_name = file.replace('_proportions.csv', '')
        ...
        for var in table_vars:
            ...
            if var not in ignore_vars:
                ...
                if not ((table[var].all() >= 0) and (table[var].all() <= 1)):
                    raise ValueError(f"Column {var} in {file} file contains values outside range [0,1]")
