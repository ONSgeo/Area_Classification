...
def sir_processing(config):
    ...
    if config["ni_disability_file"] in missing_files:
        logger.warning(
            f"Warning: The file {config['ni_disability_file']} was not found in the input directory."
        )
        convert_disability_age_group_northern_ireland(
            config["input_directory"] + config["ni_disability_input"], config
        )
    ...
    ew_disability_df = pd.read_csv(
        config["input_directory"] + config["england_wales_disability_file"]
    )
    ni_disability_df = pd.read_csv(config["input_directory"] + config["ni_disability_file"])
    scotland_disability_df = pd.read_csv(
        config["input_directory"] + config["scotland_disability_file"]
    )
    combined_disability_df = pd.concat([ew_disability_df, ni_disability_df, scotland_disability_df])
...
def SIR_calculation(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    df_nat_summary = (
        ...
        .agg(
            ...
            sum_disability_count=("total_disabled", "sum"),
        )
        .reset_index()
    )

    # Create national proportions for each age group by dividing the disability count by population
    df_nat_summary["nat_prop"] = (
        df_nat_summary["sum_disability_count"] / df_nat_summary["sum_population"]
    )

    # Join the national proportions back to the original DataFrame
    df = df.merge(
        df_nat_summary[["age_group", "nat_prop"]], on="age_group", how="left", suffixes=("", "_nat")
    )
    ...
    df_all = (
        ...
        .agg(exp_ill_all=("exp_ill", "sum"), disability_count=("total_disabled", "sum"))
        .reset_index()
    )

    # Calculate SIR for each Area Code
    df_all["SIR"] = df_all.apply(
        lambda row: round((row["disability_count"] / row["exp_ill_all"]) * 100, 4), axis=1
    )
...
def sir_qa_checks(df: pd.DataFrame, config: dict) -> None:
    ...
    if df["disability_count"].dtype != "int64":
        logger.warning("Disability count should be of type int64")
