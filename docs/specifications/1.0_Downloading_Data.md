...
2. For NI, the data is fetched from URLs and downloaded table-by-table. Metadata is fetched and downloaded. The manually downloaded population density table is reformatted to only contain relevent columns and to convert from hectare to square kilometres (km2).
