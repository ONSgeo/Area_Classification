...
for (url in zip_urls){
  ...
  t_name <- regmatches(url, regexpr("ts\\d{3}[a-z]?", url))
  
  t_tab_loc <- list.files("./tmp", pattern=paste(".", t_name, "-ltla.csv", sep = "")) # Extract the LTLA csv location
  
  if (length(t_tab_loc) == 0){
    t_tab_loc <- list.files("./tmp", pattern=paste(".", t_name, "-llta.csv", sep = "")) #ts004 has a typo
  }
