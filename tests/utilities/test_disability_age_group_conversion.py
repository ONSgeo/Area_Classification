...
class TestConvertDisabilityAgeGroupScotland(unittest.TestCase):
    def setUp(self):
        ...
        self.config = {
            ...
            "LAD_lookup_file_path": "./tests/data/scotland_lad_lookup.csv",
        }
        ...
        age_bands = (
            ["0 to 4", "5 to 9", "10 to 14", "15", "16 to 17", "18 to 19", "20 to 24"]
        ...
        non_disabled_count = [
            population[i] - (little_limited_count[i] + lot_limited_count[i])
...
class TestConvertDisabilityAgeGroupNorthernIreland(unittest.TestCase):
    def setUp(self):
        ...
        df = pd.concat(
            [placeholders_before, header, data_rows, placeholders_after], ignore_index=True
        )
        os.makedirs(os.path.dirname(self.test_data_filepath), exist_ok=True)
    ...
    def test_convert_disability_age_group_northern_ireland(self):
        result_df = convert_disability_age_group_northern_ireland(
            self.test_data_filepath, self.config
        )
        self.assertIsInstance(result_df, pd.DataFrame)
        self.assertFalse(result_df.empty)
        pd.testing.assert_frame_equal(result_df, self.expected_df)
